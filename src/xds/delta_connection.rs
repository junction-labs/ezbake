...
use crate::xds::{is_delta_nack, is_nack};

use super::{
    cache::{Entry, ResourceVersion, VersionedProto},
    ResourceNames, ResourceType, SnapshotCache,
};
...
impl AdsConnection {
    ...
    fn ads_response_for(&mut self, rtype: ResourceType) -> Option<DeltaDiscoveryResponse> {
        let Some(sub) = self.subscriptions[rtype].as_mut() else {
            return None;
        };
        ...
        // TODO: handle a wildcard update for this node
        if sub.is_wildcard {}
